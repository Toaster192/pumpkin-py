...
In our case, we only have one module specified, so we have to create a file ``bistro/module.py``. This file has to contain the class inheriting from disnake's Cog and the ``setup()`` function to load the module.
